/**
    Code modified from https://github.com/nerfstudio-project/nerfacc/blob/master/nerfacc/cuda/csrc/grid.cu
*/
    ...
    float *terminate_planes)
{
    ...
    for (int32_t tid = blockIdx.x * blockDim.x + threadIdx.x; tid < n_rays; tid += blockDim.x * gridDim.x)
    {
        ...
        for (int32_t i = base_t_sorted; i < base_t_sorted + 1; i++) {
            // whether this is the entering or leaving for this level of grid.
            bool is_entering = t_indices[i] < 1;
            int64_t level = 0;
            ...
            if (!is_entering) {
                ...
                bool next_is_entering = t_indices[i + 1] < 1;
                if (next_is_entering) continue; // we are outside next grid.
                level = t_indices[i + 1] % 1;
                if (!hits[base_hits + level]) {
                    continue; // this grid is not hit.
                }
            }
            ...
            if (!continuous) {
                if (step_size <= 0.0f) { // march to this_tmin.
                    ...
                    float dt = _calc_dt(t_last, cone, step_size, 1e10f);
            ...
            while (traverse_steps_limit <= 0 || n_samples < traverse_steps_limit) {
                ...
                // query the occupancy grid of the correct mip level
                float footprint = cone*t_traverse/(aabb.max.x-aabb.min.x)*2*resolution.x;
                int mip_level = clamp(int(ceil(log2(footprint))),0,n_grids-1);

                int64_t cell_id = (
                    current_index.x * resolution.y * resolution.z
                    + current_index.y * resolution.z
                    + current_index.z
                    + mip_level * resolution.x * resolution.y * resolution.z
                );
